...
async function locationHandler(req, res) {
    try {
        ...
        const restaurantArr = yelpResponse.body.businesses.map(
            restaurant => new Restaurant(restaurant)
        );
        responseObj.restaurantData = restaurantArr;

        url = 'https://api.openweathermap.org/data/2.5/weather';
        const openWeatherResponse = await superagent.get(url).query({
        ...
        try {
            ...
            movieTheaterArr = movieGluResponse.body.cinemas.map(
                cinema => new MovieTheater(cinema)
            );
        } catch (error) {
            console.log('MovieGlu error:', error.message);
        }
...
function Location(searchQuery, location) {
    ...
    this.formattedQuery = location.display_name;
